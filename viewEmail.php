...
<div id="main">
    <div class="row">
        ...
        if (!empty($email_data['body']['plain'])) {
            // Explode the plain text body into an array of lines
            $lines = explode("\n", $email_data['body']['plain']);

            // Initialize a flag to check for replies
            $isReply = false;
            ...
            foreach ($lines as $line) {
                ...
                // Check if the line indicates the start of a reply
                if (strpos($line, 'On') === 0) {
                    // If it's a reply, add <hr> to separate it from the previous content
                    if ($isReply) {
                        echo '<hr>';
                    }
                    $isReply = true;
